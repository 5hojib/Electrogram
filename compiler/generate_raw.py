...
API_HOME_PATH = Path("compiler/api").resolve()
REPO_HOME_PATH = Path(".").resolve()
    ...
    for kind in ["base", "types", "functions", "enums"]:
        ...
        for ns_path in [kind_path] + list(kind_path.rglob("*")):
            if ns_path.is_dir():
                ...
                sub_namespaces = []
                for d in ns_path.glob("*"):
                    if d.is_dir():
                        sub_namespaces.append(d.name)

                if not modules and not sub_namespaces:
                    continue

                with open(ns_path / "__init__.py", "w") as f:
                    f.write(f"{WARNING}\n\n")
                    ...
                    if sub_namespaces:
                        f.write(f"from . import {', '.join(sorted(sub_namespaces))}\n")
                        all_exports.extend(sorted(sub_namespaces))

                    f.write("\n\n__all__ = [\n")
                    for it in all_exports:
                        f.write(f'    "{it}",\n')
                    f.write("]\n")
